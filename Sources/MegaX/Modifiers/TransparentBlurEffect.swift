...
extension View {
    ...
    public func transparentBackgroundBlurEffect(_ transparency: LayerTransparency = .normal, smoothEdges: Edge.Set = .all) -> some View {
        modifier(TransparentBackgroundBlurEffect(transparency: transparency, smoothEdges: smoothEdges))
    }
}
...
struct TransparentBackgroundBlurEffect: ViewModifier {
...
#Preview {
    ZStack(alignment: .top) {
        ...
        Color.clear
            .frame(height: 50)
            .transparentBackgroundBlurEffect(smoothEdges: .bottom)
